...
workflow CNVAnalysis {
  ...
  call export {
    input:
      binnedBed = cnv_rescaling.outBinnedBed,
      signalToNoiseRatio = rescaling.signalToNoiseRatio,
      cnvRatiosBed = cnv_rescaling.outCnvRatiosBed,
      cnvsDetected = cnv_rescaling.cnvsDetected,
      isInputControl = cnv_rescaling.isInputControl,
      fittingParams = fitting.outParams,
      pbsBed = pbs.outBed,
      predictedEpitopes = classifying.predictedEpitopes,
      bypassCNVRescalingStep = bypassCNVRescalingStep,
      context = context,
      outJsonDir = outJsonDir,
